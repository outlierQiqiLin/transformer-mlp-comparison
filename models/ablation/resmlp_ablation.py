...
class ResMLP_Block_Attn(nn.Module):
    ...
    def __init__(self, nb_patches, dim, layerscale_init, expansion_factor=4, 
...
class ResMLP_Ablation(nn.Module):
    """
    ResMLP 消融实验模型
...
def resmlp_baseline(**kwargs):
    """原始 ResMLP"""
    return ResMLP_Ablation(variant='baseline', **kwargs)


def resmlp_with_attention(**kwargs):
    """ResMLP + Attention"""
    return ResMLP_Ablation(variant='attn', **kwargs)


def resmlp_with_layernorm(**kwargs):
    """ResMLP + LayerNorm"""
    return ResMLP_Ablation(variant='no_affine', **kwargs)


def resmlp_no_layerscale(**kwargs):
    """ResMLP without LayerScale"""
    return ResMLP_Ablation(variant='no_layerscale', **kwargs)


def resmlp_no_cross_patch(**kwargs):
    """ResMLP without cross-patch sublayer"""
    return ResMLP_Ablation(variant='no_cross_patch', **kwargs)


def resmlp_vit_like(**kwargs):
    """ResMLP with Attention + LayerNorm (类 ViT)"""
    return ResMLP_Ablation(variant='full', **kwargs)