# train_resmlp_ablation_sst2.py
"""
消融实验训练脚本
用法:
python train_resmlp_ablation_sst2.py --config config/sst2_resmlp_ablation.yaml --variant baseline
"""
...
from models.ablation.resmlp_ablation_text import ResMLP_Ablation_Text
from utils.data_loader_text import get_dataloader_sst2


def create_ablation_model(config, variant: str, vocab_size: int, device):    
    """
    为消融实验创建模型
    ...
    # 创建消融模型
    model = ResMLP_Ablation_Text(
        vocab_size=vocab_size,      # (新增)
        max_seq_len=max_seq_len,  # (新增)
...
def train_epoch(model, train_loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, batch in enumerate(pbar):
        # batch 是一个字典: {'input_ids', 'attention_mask', 'labels'}
        input_ids = batch['input_ids'].to(device)
        targets = batch['labels'].to(device)
        # attention_mask 在这个简单模型中未使用，但如果需要，可以传递
...
def main():
    parser = argparse.ArgumentParser(description='ResMLP (SST-2) 消融实验训练')
    ...
    # 6. 加载数据
    print("📊 加载数据集...")
    train_loader, test_loader, vocab_size = get_dataloader_sst2(config)
    print(f"   训练样本: {len(train_loader.dataset)}")
    print(f"   测试(验证)样本: {len(test_loader.dataset)}")
    print(f"   批次数: {len(train_loader)} (train), {len(test_loader)} (test)\n")

     # 7. 创建模型
    model = create_ablation_model(config, args.variant, vocab_size, device)
